---
name: e2e-testing
description: Master end-to-end testing to build reliable test suites that catch bugs, improve confidence, and enable fast deployment. Use when implementing E2E tests, debugging flaky tests, or establishing testing standards.
allowed-tools: Read, Write, Edit, Bash, Glob, Grep
version: 1.0.0
credits: https://github.com/wshobson/agents/tree/main/plugins/developer-essentials/skills/e2e-testing-patterns
changelog:
  - 1.0.0: Initial version — Playwright & Cypress patterns, mobile/desktop split, locale-agnostic selectors, dynamic dates
---
...
9. **No Hardcoded Dates**: Always compute dates dynamically relative to `new Date()` — hardcoded dates make tests brittle over time
10. **Locale-Agnostic Selectors**: Never rely on text tied to a single language; prefer `data-testid` or regex matching all supported locales
11. **Separate Mobile and Desktop Tests**: Write two distinct test functions per scenario — never use `if (isMobile)` or `test.skip()` inside a test body
12. **Small Reusable Functions**: Extract every meaningful interaction into a named utility function; a test body should read as a sequence of high-level steps
13. **Code Quality Gate**: Before considering a test done, ensure `typecheck`, `lint`, and `format` all pass with zero errors
14. **No Repo Pollution**: Keep exploration/debug scripts in `/tmp/`, never commit them to the project
...
await page.getByText(/select dates|choisir les dates/i).click();
```

### Pattern 5: Mobile & Desktop Split
...
- **Single-Locale Text**: Text selectors break in other languages — use `data-testid` or regex
- **Viewport Conditionals**: `if (isMobile)` in tests hides intent — split into dedicated functions
- **Monolithic Tests**: Long test bodies are hard to debug — extract into small named utilities
