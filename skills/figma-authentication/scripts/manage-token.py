...
Usage:
  ...
  python3 manage-token.py --save "figd_xxx"  # Persist a token to settings.local.json
...
Output contract:
  ...
  $FIGMA_PASSWORD directly from the environment (or source the .env file).
"""
...
def find_token_in_settings() -> tuple[str, str]:
    """Read FIGMA_TOKEN from .claude/settings.local.json → env block."""
...
def find_credentials() -> tuple[str, str, dict]:
    """Search for FIGMA_USERNAME/FIGMA_EMAIL and FIGMA_PASSWORD in env and .env files."""
...
def validate_token(token: str) -> tuple[bool, str, dict]:
    """Call /v1/me to validate the token. Retries up to 3 times on transient errors."""
    for attempt in range(3):
        try:
            req = urllib.request.Request(FIGMA_API_ME, headers={"X-Figma-Token": token})
...
def save_token_to_settings(token: str) -> tuple[bool, str]:
    """
    Write FIGMA_TOKEN into .claude/settings.local.json → env block.
...
def main():
    ...
    eprint("  To enable auto-login, add to .env.local:")
    eprint("    FIGMA_USERNAME=your@email.com")
    eprint("    FIGMA_PASSWORD=your_password")
