...
| Chaque champ a une utilité émergente, sinon c'est une redite (1.7.4) | Le titre **nomme**, le Goal porte **l'enjeu testable** (infinitif d'aboutissement + objet + enjeu ; sujet implicite = l'actrice), une ligne Capability gardée **borne**, le scénario **prouve** ; Capability optionnelle avec règle de suppression | Des Goals et Capabilities en paragraphes entiers observés en review : un champ sans rôle différencié se remplit en paraphrasant ; le goal est un intrant (liste actor-goal confirmée), jamais réécrit pour coller au flux — sinon dérive de périmètre sans gate | Cockburn 2000 (use cases nommés par un verbe, « go away satisfied ») ; review PM 2026-09 | durable | — |
