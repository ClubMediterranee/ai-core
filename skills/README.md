...
| `sync-specs` | Product | Detects and documents synchronisation points between specs of different PRDs — shared endpoints, editorial keys, and mechanisms no single spec owns. Joins specs on their §8 Data Contract keys, then records a synchro or extracts a transversal feature, and proposes citations to add to the carrier specs. |
